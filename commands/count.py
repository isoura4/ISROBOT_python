...
class count(commands.Cog):
    ...
    async def count(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        if not DB_PATH:
            await interaction.response.send_message("Le chemin de la base de données n'est pas défini dans les variables d'environnement.", ephemeral=True)
            return
        
        # Vérifier si le salon est valide
        if not isinstance(channel, discord.TextChannel):
            await interaction.response.send_message("Veuillez spécifier un salon texte valide.", ephemeral=True)
            return
        ...
        if existing:
            await interaction.response.send_message(f"Le minijeux du compteur est déjà configuré dans le salon {channel.mention}.", ephemeral=True)
            conn.close()
            return
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (str(interaction.guild.id), str(channel.id), '', str(interaction.user.id), '0', 0))

        conn.commit()
        conn.close()

        await interaction.response.send_message(f"Le minijeux du compteur a été configuré dans le salon {channel.mention}.", ephemeral=True)
